...
QWEEN_BUILD_GPU_DIR="$CORE_DIR/kali_core/voice/qwen_cpp/build-gpu"
if [ "${KALI_TTS_PROVIDER:-inproc}" = "qwen3" ] || [ "${KALI_TTS_PROVIDER:-inproc}" = "qwen3-voicedesign" ]; then
  if [ ! -f "$QWEN_BUILD_DIR/tts-server" ] && [ ! -f "$QWEEN_BUILD_GPU_DIR/tts-server" ]; then
    echo "WARNING: Qwen3-TTS binary not found."
    echo "  To compile:  ./scripts/build-qwen-cpp.sh cpu   (or cuda for GPU)"
    echo "  To download: ./scripts/download-qwen-models.sh"
    echo "  Falling back to Piper (inproc)."
